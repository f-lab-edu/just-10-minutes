...
public interface ProductMapper {
    ...
    @Insert("INSERT INTO products_table (title, description, seller_id, original_price, total_stock, purchased_stock) " +
    ...
    @Select("SELECT * FROM products_table WHERE id = #{id}")
    public ProductEntity findById(final Long id);

    @Select("SELECT * FROM products_table WHERE id = #{id} FOR UPDATE")
    public ProductEntity findByIdForUpdate(final Long id);

    @Update("UPDATE products_table SET purchased_stock = #{updatedStock} WHERE id = #{id}")
    public int patchStock(final Long id, final Long updatedStock);

    @Delete("DELETE FROM products_table WHERE id = #{id}")
